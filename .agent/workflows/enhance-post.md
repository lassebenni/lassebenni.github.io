...
3. **Identify sections to wrap in accordions**:
    - Look for major sections (typically H3 headings) that could benefit from being collapsible.
    - Wrap these sections in accordion shortcodes using the syntax: `{{% accordion title="Section Title" level="3" %}}`
    - Ensure the accordion title matches the original heading text.
    - Place the closing `{{% /accordion %}}` tag after the section content.
    - This makes the post more scannable and adds the sections to the Table of Contents.
4. Update the YAML front matter of the post:
    - Add `ai_summary: "..."`.
    - Add `tags: [...]`.
...
5. Verify that the file remains valid Hugo content and the front matter is correctly formatted.
